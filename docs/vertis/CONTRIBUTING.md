...
2. **Code only goes upstream.** Nothing under `.planning/` or `docs/vertis/`, and no internal notes. Documentation changes go upstream only when they describe the shipped system. Before pushing an `upstream/*` branch, confirm the folder is absent:
   ```bash
   git ls-tree -r --name-only HEAD | grep -q '^docs/vertis/' && echo 'STOP: docs/vertis is in this branch' || echo ok
   ```
