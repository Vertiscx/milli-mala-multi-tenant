...
│  │ `src/platform/tenant.ts`        │   │ `src/services/archive/webhook.ts`        │                │
│  │ - KvTenantStore        │   │ `src/services/archive/cases.ts`          │                │
│  │ - FileTenantStore      │   │ `src/services/archive/attachments.ts`    │                │
│  │ - validateTenantConfig │   │ Return { status, body } │                │
│  └────────────────────────┘   └──────────┬─────────────┘                │
│                                          │                              │
│                                          ▼                              │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │ Pipeline stages (`src/services/archive/documentTicket.ts`)                       │    │
...
│  │ `src/platform/zendesk.ts`     │   │ `src/services/archive/docClient.ts`                      │ │
...
│  │ `src/services/archive/pdf.ts`     │    │ `src/platform/fileAuditStore.ts` (Node) /  │         │
...
| Tenant store + validators | Resolve `brand_id` → `TenantConfig`, SSRF/subdomain guards | `src/platform/tenant.ts` |
| Tenant config builder | Define which tenants exist, pull secrets from env vars | `src/tenants.config.ts` |
| Webhook handler | HMAC verify + freshness + delegate to pipeline | `src/services/archive/webhook.ts` |
| Cases handler (manual create/append) | GW-06 envelope, exactly-one-of `create` XOR `case_number`, orphan_case safety | `src/services/archive/cases.ts` |
| Attachments handler (manual forwarding) | Fetch ZD attachments, forward each via `docClient.uploadDocument` | `src/services/archive/attachments.ts` |
| Documentation pipeline | Composable stages (fetch/render/upload), shared by all 3 handlers | `src/services/archive/documentTicket.ts` |
| Result post-back (GW-01) | Best-effort internal note + custom-field stamping after each outcome | `src/services/archive/postResultToTicket.ts` |
| DocClient factory | Build the right adapter from `EndpointConfig.type` | `src/services/archive/docClient.ts` |
| OneSystems adapter | `authenticate`, `uploadDocument` (multipart), `createCase` | `src/services/archive/onesystems.ts` |
| GoPro adapter | `authenticate`, `uploadDocument` (per-file POSTs) — no `createCase` | `src/services/archive/gopro.ts` |
| Zendesk client | REST GET helpers + `requestWrite` (PUT/POST) + `setTicketCustomField` | `src/platform/zendesk.ts` |
| PDF generator | Render `ZendeskTicket + ZendeskComment[]` → `Buffer` (jsPDF) | `src/services/archive/pdf.ts` |
| Audit store (Node) | File-backed `AuditStore` (KV-shaped interface) | `src/platform/fileAuditStore.ts` |
| Logger | Structured JSON to stdout (severity/component/timestamp) | `src/platform/logger.ts` |
| Instance config | Non-tenant runtime config (port, log level, audit secret) | `src/platform/config.ts` |
| Env reader | `requireEnv` fail-fast for tenant secrets | `src/platform/env.ts` |
| Shared types | `TenantConfig`, `EndpointConfig`, `DocClient`, `DocumentationOutcome`, … | `src/platform/types.ts` + `src/services/archive/types.ts` |
...
**Key Characteristics:**
- **Adapter pattern at exactly one seam.** `createDocClient(ep)` in `src/services/archive/docClient.ts` is the ONLY place that switches on `ep.type`. Every other layer programs against the `DocClient` interface in `src/services/archive/types.ts`.
- **Capability checks are duck-typed, never `ep.type`-typed.** See `src/services/archive/cases.ts:178-180` — `typeof (docClient as Partial<OneSystemsClient>).createCase === 'function'`. Add new adapter capabilities by adding a method, not a `type` branch.
...
- Location: `src/platform/tenant.ts`, `src/tenants.config.ts`
...
- Location: `src/services/archive/webhook.ts`, `src/services/archive/cases.ts`, `src/services/archive/attachments.ts`
...
- Location: `src/services/archive/documentTicket.ts`
...
- Location: `src/services/archive/onesystems.ts`, `src/services/archive/gopro.ts`; constructed via `src/services/archive/docClient.ts`
...
- Location: `src/services/archive/postResultToTicket.ts`, `writeAudit` in `src/services/archive/documentTicket.ts`
...
3. **Tenant resolution** — `resolveTenantConfig(brand_id, store)` (`src/platform/tenant.ts:86`). Returns `null` on missing/invalid → 400.
4. **Handler gate** (`src/services/archive/cases.ts:74-134`):
...
5. **`fetchTicketInfo`** (`src/services/archive/documentTicket.ts:51`) — ticket + comments + attachments + author resolution + brand cross-check. Returns the GW-06 `brand_mismatch` envelope on mismatch.
6. **`renderPdf`** (`src/services/archive/documentTicket.ts:109`) — jsPDF.
7. **`createDocClient(ep, solvingAgentEmail)`** (`src/services/archive/docClient.ts:14`) — **the only `ep.type` branch in the codebase**.
...
10. **`postToCase`** (`src/services/archive/documentTicket.ts:153`) — `docClient.uploadDocument({ caseNumber, filename, pdfBuffer, attachments, metadata })`.
...
12. **`recordOutcome`** (`src/services/archive/postResultToTicket.ts:205`) — writes audit entry + PUTs ticket with internal note + custom fields (`caseNumberFieldId`, `templateFieldId`, `lastStatusFieldId`, `lastExportFieldId`). Best-effort.
...
3. `ZendeskClient.getTicket` + brand cross-check (`src/services/archive/attachments.ts:114-133`).
...
2. `verifyWebhookSignature` (HMAC-SHA256, `timingSafeEqual`) + `isTimestampFresh` (±5 min) — `src/services/archive/webhook.ts:19-39`.
3. Delegate to `documentTicket(req, ticketId, startTime)` — the full pipeline orchestrator (`src/services/archive/documentTicket.ts:274`).
...
**`TenantConfig`** (`src/platform/types.ts:7`):
...
**`EndpointConfig`** (`src/platform/types.ts:31`):
...
**`DocClient` interface** (`src/services/archive/types.ts:13`):
...
**`DocumentationOutcome`** (`src/services/archive/types.ts:53`):
...
- Tenant data: written to KV with key `tenant:${brandId}` (see `KvTenantStore.get`, `src/platform/tenant.ts:48`)
...
- **Global state:** `config` cache in `src/platform/config.ts:15` (module-level, reset via `resetConfig()` for tests). No other module-level mutable state.
...
- **Cloudflare nodejs_compat REQUIRED** (`wrangler.toml:4`) — `src/services/archive/webhook.ts` and `src/worker.ts` use `node:crypto` for HMAC and `timingSafeEqual`.
- **Body size hard cap: 1 MB** (both runtimes). Enforced before parse to limit DoS surface.
- **Outgoing `baseUrl` SSRF guard** (`src/platform/tenant.ts:160-183`) — HTTPS-only, no private/loopback/IPv6 literals. Applies to ALL endpoint types; new adapters inherit it for free.
- **Subdomain pattern** (`src/platform/tenant.ts:16`) — Zendesk subdomain must match `/^[a-z0-9][a-z0-9-]*$/i` to prevent URL injection.
...
**Do this instead:** Add a method to the adapter (or omit it) and duck-type. See `src/services/archive/cases.ts:178-180`:
```ts
const canCreateCase =
...
**What happens:** A change inside `src/services/archive/postResultToTicket.ts` lets an error escape.
...
**Do this instead:** Wrap any new logic in try/catch + `logger.warn(...)`. See `src/services/archive/postResultToTicket.ts:188-194`.
...
**Why it's wrong:** `src/services/archive/cases.ts:47-55` says verbatim: *"Copied verbatim from src/services/archive/attachments.ts:21-29 (do NOT import/share — src/services/archive/attachments.ts must stay byte-identical)."* The duplication is load-bearing for the G4 byte-identical guarantee.
...
**Do this instead:** Match the structure in `src/services/archive/cases.ts:229-316` — separate inner try/catch keyed off `createdCaseNumber !== undefined` returns 207 `orphan_case` carrying `caseNumber`.
...
- HMAC signatures: `createHmac('sha256', secret).update(timestamp + rawBody).digest('base64')` then `timingSafeEqual` (`src/services/archive/webhook.ts:19-29`).
- Adapter errors carry HTTP status in the message text (e.g. `OneSystems upload failed: ${status} - ${errorText}`). Never leak bearer tokens in messages (`src/services/archive/onesystems.ts:167`).

## Cross-Cutting Concerns

**Logging:** Structured JSON via `createLogger(component)` (`src/platform/logger.ts`). Levels: debug/info/warn/error. All log lines include `severity`, `component`, `message`, `timestamp`. Tenant identifiers are logged as `brand_id` (a public Zendesk ID, not a secret).

**Validation:** Centralized in `src/platform/tenant.ts` (subdomain, baseUrl, case_number, audit params) and inline in each handler (ticket_id, GW-06 envelope shape). Validation always precedes adapter construction.
...
- Zendesk outbound → Basic `${email}/token:${apiToken}` (`src/platform/zendesk.ts:14-16`).

**SSRF guard:** `validateEndpoint` (`src/platform/tenant.ts:160-183`) blocks any `baseUrl` that resolves to private/reserved hostnames. Applies before adapter construction, so it covers every adapter type uniformly.

**Audit writes:** `writeAudit` (`src/services/archive/documentTicket.ts:177`) writes one record under TWO keys (`audit:${brandId}:${ts}:${ticketId}` and `ticket:${brandId}:${ticketId}:${ts}`) with `expirationTtl: 90 * 24 * 60 * 60` (90 days). Webhook path passes no enrichment args so the persisted shape stays byte-identical to the pre-G4 entry.
